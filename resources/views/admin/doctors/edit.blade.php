            ...
            <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
                ...
                <div class="d-flex align-items-center flex-wrap mr-1">
                    ...
                    <div class="d-flex align-items-baseline flex-wrap mr-5">
                        <!--begin::Page Title-->
                        <h5 class="text-dark font-weight-bold my-1 mr-5">@lang('site.doctors')</h5>
                        <!--end::Page Title-->
                        <!--begin::Breadcrumb-->
                        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                            <li class="breadcrumb-item text-muted">
                                <a href="{{route('admin.doctors.index')}}" class="text-muted">@lang('site.doctors')</a>
                            </li>
                            <li class="breadcrumb-item text-muted">
        ...
        <div class="d-flex flex-column-fluid">
            ...
            <!--begin::Card-->
                <div class="card card-custom gutter-b example example-compact" data-card="true">
                    ...
                    <!--begin::Form-->
                    <form id="validatable-form" class="form validatable-form" method="post" action="{{ route('admin.doctors.update', $doctor->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-lg-12">
                                    ...
                                    <select name="doctor_category_id" class="form-control" id="form_select">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}" {{ $category->id == $doctor->doctor_category_id ? 'selected' : ''  }}>{{$category->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                {{--doctor_services--}}
                                <div class="form-group col-lg-12" id="products">
                                    ...
                                    <div class="checkbox-inline">
                                        @foreach($doctor_services as $d_service)
                                            <label class="checkbox">
                                                <input value="{{$d_service->id}}" type="checkbox" name="doctor_services[]" checked>
                                        ...
                                        <div class="tab-pane lang-tab-pane fade {{($locale == 'ar') ? 'show active' : ''}}" id="{{$locale}}" data-locale="{{$locale}}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input name="{{$locale}}[name]" class="form-control lang_fields_{{$locale}}" type="text" value="{{ $doctor->translate($locale)->name }}" placeholder="أكتب العنوان" required="required">
                                                </div>
                                            ...
                                            <div class="form-group col-lg-12">
                                                ...
                                                <textarea name="{{$locale}}[description]" id="{{$locale}}_my_tiny" rows="3" placeholder="أكتب الوصف" class="form-control">{!! $doctor->translate($locale)->description !!}</textarea>
                                            </div>
                                            {{--meta_title--}}
                                            <div class="form-group">
                                                <label class=" ">@lang('site.' . $locale . '.meta_title')
                                                    <span class="m-1 text-primary la la-language" data-toggle="tooltip" title="هذة الحقول لها قيم مختلفة لكل لغة"></span>
                                                </label>
                                                <div class="input-group">

                                                    <input name="{{$locale}}[meta_title]" class="form-control lang_fields_{{$locale}}" type="text" value="{{$doctor->translate($locale)->meta_title}}" placeholder="أكتب @lang('site.' . $locale . '.meta_title')">
                                                </div>
                                                <span class="form-text text-muted">قم بكتابة @lang('site.' . $locale . '.meta_title')</span>
                                            </div>
                                            {{--meta_description--}}
                                            <div class="form-group col-lg-12">
                                                <label>@lang('site.' . $locale . '.meta_description')
                                                    <span class="m-1 text-primary la la-language" data-toggle="tooltip" title="هذة الحقول لها قيم مختلفة لكل لغة"></span>
                                                </label>
                                                <textarea name="{{$locale}}[meta_description]" rows="3" placeholder="أكتب @lang('site.' . $locale . '.meta_description')" class="form-control lang_fields_{{$locale}}">{!! $doctor->translate($locale)->meta_description !!}</textarea>
                                                <span class="form-text text-muted">قم بكتابة @lang('site.' . $locale . '.meta_description')</span>
                                            </div>
                                            {{--keywords--}}
                                            <div class="form-group">
                                                <label class=" ">@lang('site.' . $locale . '.keywords')
                                                    <span class="m-1 text-primary la la-language" data-toggle="tooltip" title="هذة الحقول لها قيم مختلفة لكل لغة"></span>
                                                </label>
                                                <div class="input-group">

                                                    <input name="{{$locale}}[keywords]" class="form-control lang_fields_{{$locale}}" type="text" value="{!! $doctor->translate($locale)->keywords !!}" placeholder="أكتب @lang('site.' . $locale . '.keywords')">
                                                </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input name="{{$locale}}[image_alt]" class="form-control lang_fields_{{$locale}}" type="text" value="{!! $doctor->translate($locale)->image_alt !!}" placeholder="أكتب @lang('site.' . $locale . '.image_alt')">
                                                </div>
                                ...
                                {{--image--}}
                                <div class="form-group col-lg-12">
                                    <label for="image">@lang('site.doctor_image')</label>
                                    <div class="col-lg-9 col-xl-6">
                                        <div class="image-input image-input-outline image-input-circle" id="kt_image_3">
                                            <div class="image-input-wrapper" style="background-image: url({{($doctor->image ? url('/thumbnail/120/120/80/storage/doctor_images/'.$doctor->image) : 'https://ui-avatars.com/api/?name=مثال&color=000&background=03A9F4')}});"></div>
...
@push('scripts')
    ...
    {{--image--}}
    <script src="{{asset('admin/assets/js/doctors/crud/file-upload/image-input.js?v=7.2.9')}}"></script>

    <script>
        $(document).ready(function(){
            $("#form_select").change(function(e){
                ...
                $.ajax({
                    url: "{{route('admin.doctor_services.get')}}",
                    type:'get',
                    data: {
                        'id':id,
                        _token: _token
